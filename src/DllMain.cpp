...
Game::LoadScriptFunctions_t LoadScriptFunctions_o = nullptr;
Game::RenderObjectBlips_t RenderObjectBlips_o = nullptr;
Game::ObjectEnumProc_t ObjectEnumProc_o = nullptr;
Game::ClntObjMgrEnumVisibleObjects_t ClntObjMgrEnumVisibleObjects_o = nullptr;
...
std::unordered_map<uint64_t, TrackedUnit> TrackedUnitsBlip;
std::unordered_map<uint64_t, Game::C2Vector> TrackedUnitsMinimapPos;
std::unordered_map<std::string, Game::HTEXTURE__ *> TextureCache;
...
static int __cdecl Script_WriteFile(void *L) {
    ...
    auto filename = Game::Lua::ToString(L, 1);
    auto mode = Game::Lua::ToString(L, 2);
    auto content = Game::Lua::ToString(L, 3);
...
static int __cdecl Script_SetUnitBlip(void *L) {
    ...
    auto unitName = Game::Lua::ToString(L, 1);
    ...
    if (!Game::Lua::IsString(L, 2)) {
        TrackedUnitsBlip.erase(unitGUID);
        return 0;
    }

    // TODO: Path should be lower cased to avoid duplicates
    auto texturePath = Game::Lua::ToString(L, 2);

    Game::HTEXTURE__ *texture;

    const auto it = TextureCache.find(texturePath);
    ...
    TrackedUnitsBlip[unitGUID] = {texture, scale};

    return 0;
}

static void TrackUnit(Game::MINIMAPINFO *info, uint64_t guid) {
    const auto it = TrackedUnitsBlip.find(guid);
    if (it == TrackedUnitsBlip.end())
        return;
    ...
    if (unitptr != nullptr) {
        ...
        // 0x670540
        TrackedUnitsMinimapPos[guid] = minimapPos;
    }
}
...
static void DrawTrackedBlips(Game::CGMinimapFrame *minimapPtr, Game::DNInfo *dnInfo) {
    ...
    for (const auto &[guid, minimapPos] : TrackedUnitsMinimapPos) {
        const auto it = TrackedUnitsBlip.find(guid);
        if (it == TrackedUnitsBlip.end())
            continue;
